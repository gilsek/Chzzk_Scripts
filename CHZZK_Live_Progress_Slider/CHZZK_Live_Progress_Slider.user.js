...
(function() {
    'use strict';
    if (location.href.startsWith("https://chzzk.naver.com/embed/")) {
        return;
    }
    ...
    GM_addStyle(`
        ...
        body.live_no_time_machine #live_player_layout .pzp-progress-slider .pzp-ui-progress__played {
            background: var(--tnp--seekbar-color-fill-primary) !important;
        }
        .pzp-pc-vod-time.pzp-pc__vod-time {
            margin-left:20px;
            padding-top: 2px;
            cursor: pointer;
        }
    ...
    function createProgressBar(video) {
        document.querySelector('body').classList.remove("live_no_time_machine");
        ...
        // check time-machine
        const live_time_button = document.querySelector('.live_time_button');
        if (live_time_button.disabled) {
            // not use time-machine
            document.querySelector('body').classList.add("live_no_time_machine");
        } else {
            // use time-machine
            //document.querySelector('body').classList.remove("live_no_time_machine");
            return;
        ...
        if (!document.querySelector('.pzp-pc__vod-time')) {
            ...
            const liveTimeContainer = document.querySelector('.live_time');
            if (liveTimeContainer) {
                ...
                liveTimeContainer.parentNode.insertBefore(newElement, liveTimeContainer.nextSibling);
